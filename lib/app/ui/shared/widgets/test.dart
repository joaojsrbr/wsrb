  ...
  void layoutChildSequence() {
    ...
    for (int column = leadingColumn; column <= trailingColumn; column++) {
      ...
      for (int row = leadingRow; row <= trailingRow; row++) {
        ...
        final parent = parentDataOf(child);

        parent.layoutOffset = Offset(
          xLayoutOffset,
          yLayoutOffset,
        );

        yLayoutOffset += child.size.height;

        childWidth += child.size.width > 200 ? child.size.width / 2 : 0;
      }

      xLayoutOffset += 200 + childWidth;
    }
