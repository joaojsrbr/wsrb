...
class NeoxSource extends RSource {
  ...
  /// Scraping [Element]
  Element? _getElementOnly(List<Element> postContentItem, String contains, [String? querySelector]) {
    bool firstWhereGenres(Element element) {
      final result = element.querySelector('h5')?.text.trim().toLowerCase().contains(contains);
      return result ?? false;
    }

    return postContentItem.firstWhereOrNull(firstWhereGenres)?.querySelector(querySelector ?? '.summary-content');
  }
  ...
  String? _setAlternativeTitle(String? oldAlternativeTitle, List<Element> postContentItem) {
  ...
  Future<Result<Content>> getData(Content content) async {
    try {
      ...
      final response = await Future.wait([
        contentRepository._dio.get(book.url, responseType: ResponseType.plain, headers: App.HEADERS),
        contentRepository._dio.post("${book.url}ajax/chapters/", responseType: ResponseType.plain, headers: App.HEADERS),
      ]);

      final Document bookDocument = parse(response.first.data);

      final ScrapingUtil scrapingUtil = ScrapingUtil.bySelector(bookDocument, selector: '.site-content');

      if (scrapingUtil.error) {
        throw BookGetDataException(message: "Error ao buscar o livro documento possivelmente nulo");
      }

      final List<Element> postContentItem = scrapingUtil.querySelectorAll('.post-content_item');
      ...
      //
      //
      final List<Element> elements = parse(response[1].data).querySelectorAll('.wp-manga-chapter');

      for (final element in elements) {
        final ScrapingUtil scrapingUtil = ScrapingUtil(element);
        final String url = scrapingUtil.getURL(selector: 'div a');

        final String title = scrapingUtil.element.children.firstWhere((element) => element.text.trim().isNotEmpty).text.trim();
      ...
      final newBook = book.copyWith(originalImage: image, type: type, score: score, alternativeTitle: alternativeTitle, sinopse: sinopse, status: status);
  ...
  Future<bool> loadData() async {
    ...
    try {
      final String subKey = 'page/${contentRepository.index}/?s&post_type=wp-manga&m_orderby=${contentRepository.config.orderBy.label}';
      ...
      for (final element in element.children) {
        ...
        final String? mediumImage = scrapingUtil.getImage(selector: 'img', bySrcSet: true, last: true).isEmptyOrNull;
        final String? largeImage = scrapingUtil.getImage(selector: 'img', bySrcSet: true).isEmptyOrNull;
  ...
  Future<Result<List<Data>>> getContent(Release release) async {
    ...
    try {
      ...
      final Response response = await contentRepository._dio.get(chapter.url, headers: App.HEADERS, responseType: ResponseType.plain);
