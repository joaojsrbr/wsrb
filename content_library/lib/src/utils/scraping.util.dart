...
class ScrapingUtil {
  ...
  ScrapingUtil.bySelector(Document document, {String? selector}) {
    final result = document.querySelector(selector ?? 'div.site-content');
    if (result != null) {
      element = result;
      error = false;
    } else {
  ...
  //       element.querySelectorAll(selector ?? '.post-content_item');
  //   if (endFirstReturn != null) {
  //     return endFirstReturn.call(elements, initialData);
  //   }
  //   if (elements.isEmpty) return null;
  //   elements.removeWhere(
  //     (element) =>
  //         whereTest?.call(element) ??
  //         _defaultRemoveWhere(
  //             element: element,
  ...
  //   if (elements.isEmpty) return null;
  //   final String? data =
  //       elements.single.querySelector(endSelector ?? '.summary-content')?.text;
  ...
  // bool _defaultRemoveWhere(
  //     {required Element element,
  //     required String Function(String? remove)? stringRemove,
  //     String? selector}) {
  ...
  String _bySrcSet(String src, [bool? last]) {
    ...
    final lista = '$src,'.replaceAll(RegExp(r'([1-9])\w+,'), '').trim().split(' ')
      ..removeWhere((element) => element.isEmpty);
    // final _ = '$src,'.replaceAll(RegExp(r'([1-9])\w+,'), '').trim().split(' ');
    final w = reges
        .allMatches('$src,')
        .map(
          (e) => int.parse('$src,'.substring(e.start, e.end).replaceAll(r'w', '').trim()),
        )
        .toList();
